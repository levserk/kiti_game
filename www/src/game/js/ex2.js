...
var topText = new PIXI.Text(
  "",
  new PIXI.TextStyle({ fontSize: "11pt", fill: "#ffffff" })
);
topText.x = 20;
topText.y = 0;
uiLayer.addChild(topText);
var bottomText = new PIXI.Text(
  "",
  new PIXI.TextStyle({ fontSize: "11pt", fill: "#ffffff" })
);
bottomText.x = 20;
...
var ground = world.createBody({
  // The confinement area for our sandbox
  userData: {
    myType: "boundary",
    label: "ground",
  },
...
const topRight = planck.Vec2(
  (renderer.screen.width - 20) * MetersPerPixel,
  20 * MetersPerPixel
);
const bottomLeft = planck.Vec2(
  20 * MetersPerPixel,
  (renderer.screen.height - 40) * MetersPerPixel
);
const bottomRight = planck.Vec2(
  (renderer.screen.width - 20) * MetersPerPixel,
  (renderer.screen.height - 40) * MetersPerPixel
);
  ...
  for (var body = world.getBodyList(); body; body = body.getNext()) {
    ...
    for (
      ...
      if (shape.getType() == "edge" && userData.myType == "boundary") {
        ...
        boundaryGraphics.moveTo(
          shape.m_vertex1.x * PixelsPerMeter,
          shape.m_vertex1.y * PixelsPerMeter
        );
        boundaryGraphics.lineTo(
          shape.m_vertex2.x * PixelsPerMeter,
          shape.m_vertex2.y * PixelsPerMeter
        );
      }
    }
  }
...
loader.add("bunny", "bunny.png").load((loader, resources) => {
  requestAnimationFrame(step);
});
// Input
...
document.addEventListener("keydown", (ev) => {
  ...
  if (ev.code == "Space") {
    ...
    var force = planck
      .Vec2(forceStrength, forceStrength)
      .mul(deltaTime * PixelsPerMeter * o.body.m_mass);
...
class GameObject {
  constructor(opts) {
    ...
    this.sprite =
      typeof opts.texture == "string"
        ? new PIXI.Sprite.from(opts.texture)
        : new PIXI.Graphics();
    ...
    this.body = this.world.createBody({
      ...
      position: {
        x: opts.position.x * MetersPerPixel,
        y: opts.position.y * MetersPerPixel,
      },
      userData: {
        // We assign some userData for this body for future handling
        gameObject: true,
      },
    });
    ...
    if (this.sprite instanceof PIXI.Sprite) {
      ...
      this.sprite.scale.set(
        opts.radius * MetersPerPixel * (PixelsPerMeter / 12),
        opts.radius * MetersPerPixel * (PixelsPerMeter / 12)
      );
    }
  ...
  click() {
    ...
    var force = planck
      .Vec2(fx, fy)
      .mul(100 * forceStrength * deltaTime * PixelsPerMeter);
    // Force wake event
  ...
  integrate(alpha) {
    ...
    this.container.x = interpolation
      ? this.body.getPosition().x * alpha * PixelsPerMeter +
        this.previousState.position.x * (1 - alpha) * PixelsPerMeter
      : this.body.getPosition().x * PixelsPerMeter;
    this.container.y = interpolation
      ? this.body.getPosition().y * alpha * PixelsPerMeter +
        this.previousState.position.y * (1 - alpha) * PixelsPerMeter
      : this.body.getPosition().y * PixelsPerMeter;
    ...
    if (
      p.x > renderer.screen.width * MetersPerPixel ||
      p.x < 0 ||
      p.y > renderer.screen.height * MetersPerPixel ||
      p.y < 0
    )
      this.dirty = true;
    ...
    if (drawLines) {
      ...
      if (this.shapeType != "circle") {
        ...
        for (
          ...
          this.debug.moveTo(
            shape.getVertex(0).x * PixelsPerMeter,
            shape.getVertex(0).y * PixelsPerMeter
          );
          for (var v = 1; v < shape.m_count; v++) {
            this.debug.lineTo(
              shape.getVertex(v).x * PixelsPerMeter,
              shape.getVertex(v).y * PixelsPerMeter
            );
          }
          this.debug.lineTo(
            shape.getVertex(0).x * PixelsPerMeter,
